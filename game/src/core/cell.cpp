...
int Cell::GetMoveCost(int class_idx) {
  return terrain_->GetMoveCost(class_idx);
}

int Cell::GetTerrainEffect(int class_idx) {
  return terrain_->GetEffect(class_idx);
}
...
int Cell::ApplyTerrainEffect(int class_idx, int value) {
  return value * terrain_->GetEffect(class_idx) / 100;
}

std::string Cell::GetTerrainName() const {
  return terrain_->GetName();
}

bool Cell::IsUnitPlaced() const {
